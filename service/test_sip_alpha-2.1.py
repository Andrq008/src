    ...
    for i in sip['data']:
        if 'UNKNOWN' in i['state']:
            ...
            with open(fr"/mnt/sip/{i['id']}.sip", "w") as file:
                file.write('')
                bot.send_message(CHAT_ID_1, f"*SIP: *  Номер *{i['id']}* Отключен на АТС SIP{NUM_ATC}")
                bot.send_message(CHAT_ID_2, f"*SIP: *  Номер *{i['id']}* Отключен на АТС SIP{NUM_ATC}")
            print(i['id'])
        if  'OK' in i['state']:
            path = pathlib.Path(fr"/mnt/sip/{i['id']}.sip")
            if path.exists():
                pathlib.Path(path.absolute()).unlink()
                bot.send_message(CHAT_ID_1, f"*SIP: *  Номер *{i['id']}* Подключен на АТС SIP{NUM_ATC}")
                bot.send_message(CHAT_ID_2, f"*SIP: *  Номер *{i['id']}* Подключен на АТС SIP{NUM_ATC}")

from pathlib import Path
criticalTime = arrow.utcnow().shift(hours=-2)
